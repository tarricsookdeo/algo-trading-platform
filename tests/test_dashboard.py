...
def mock_exec():
    ...
    ea._adapters = {}  # not an OrderRouter — no sub-adapters
    ea._tracked_orders = {"order-1": {}}
    ea._order_details = {"order-1": {"symbol": "AAPL", "side": "buy", "order_type": "market", "quantity": "10", "asset_class": "equity"}}
...
@pytest.fixture
async def client(bus, mock_data_manager, mock_exec, mock_strategy_manager, risk_manager):
    app, _ = await create_app(
        bus,
...
class TestStatus:
    ...
    async def test_status_without_data_manager(self, bus):
        app, _ = await create_app(bus)
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
...
class TestPortfolio:
    async def test_get_portfolio(self, client, mock_exec):
    ...
    async def test_get_portfolio_no_exec(self, bus, mock_data_manager):
        app, _ = await create_app(bus, data_manager=mock_data_manager)
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
...
class TestOrders:
    ...
    async def test_cancel_order(self, client, mock_exec):
        resp = await client.post("/api/orders/order-1/cancel")
        assert resp.status_code == 200
        assert resp.json()["status"] == "cancel_requested"
        mock_exec.cancel_order.assert_awaited_once_with("order-1")

    async def test_cancel_order_no_exec(self, bus, mock_data_manager):
        app, _ = await create_app(bus, data_manager=mock_data_manager)
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.post("/api/orders/order-1/cancel")
...
class TestStrategies:
    ...
    async def test_start_strategy(self, client, mock_strategy_manager):
        resp = await client.post("/api/strategies/sma_cross/start")
        assert resp.status_code == 200
        assert resp.json()["status"] == "started"
        mock_strategy_manager.start_strategy.assert_awaited_once_with("sma_cross")

    async def test_stop_strategy(self, client, mock_strategy_manager):
        resp = await client.post("/api/strategies/sma_cross/stop")
        assert resp.status_code == 200
        assert resp.json()["status"] == "stopped"
        mock_strategy_manager.stop_strategy.assert_awaited_once_with("sma_cross")

    async def test_strategies_no_manager(self, bus, mock_data_manager):
        app, _ = await create_app(bus, data_manager=mock_data_manager)
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
...
class TestRisk:
    ...
    async def test_get_risk_violations(self, client):
        resp = await client.get("/api/risk/violations")
        assert resp.status_code == 200
        assert "violations" in resp.json()

    async def test_risk_no_manager(self, bus, mock_data_manager):
        app, _ = await create_app(bus, data_manager=mock_data_manager)
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.get("/api/risk")
...
class TestPnl:
    ...
    async def test_pnl_no_managers(self, bus, mock_data_manager):
        app, _ = await create_app(bus, data_manager=mock_data_manager)
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.get("/api/pnl")
        data = resp.json()
...
class TestDataIngestion:
    async def test_ingest_bar(self, client, mock_data_manager):
        bar_data = {
            "symbol": "AAPL",
            "open": 185.0,
            "high": 186.0,
        ...
        resp = await client.post("/api/data/bars", json=bar_data)
        assert resp.status_code == 200
        assert resp.json()["ingested"] == 1

    async def test_ingest_bars_batch(self, client, mock_data_manager):
        bars = [
            {
                "symbol": "AAPL",
                "open": 185.0,
        ...
        resp = await client.post("/api/data/bars", json=bars)
        assert resp.status_code == 200
        assert resp.json()["ingested"] == 2

    async def test_ingest_quote(self, client, mock_data_manager):
        quote_data = {
            "symbol": "AAPL",
            "bid_price": 185.0,
            "bid_size": 100,
        ...
        resp = await client.post("/api/data/quotes", json=quote_data)
        assert resp.status_code == 200
        assert resp.json()["ingested"] == 1

    async def test_ingest_trade(self, client, mock_data_manager):
        trade_data = {
            "symbol": "AAPL",
            "price": 185.25,
            "size": 100,
        ...
        resp = await client.post("/api/data/trades", json=trade_data)
        assert resp.status_code == 200
        assert resp.json()["ingested"] == 1
    ...
    async def test_data_providers(self, client):
        resp = await client.get("/api/data/providers")
        assert resp.status_code == 200
        data = resp.json()
