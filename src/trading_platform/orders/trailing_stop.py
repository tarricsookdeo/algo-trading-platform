"""Trailing stop order manager.

Monitors bid prices and ratchets a resting stop-loss order upward as the
price moves in favor. Uses cancel_and_replace for efficient atomic stop
price updates.
"""

from __future__ import annotations

import uuid
from datetime import datetime, timezone
from decimal import Decimal
from enum import StrEnum
from typing import Any

from pydantic import BaseModel, Field

from trading_platform.adapters.base import ExecAdapter
from trading_platform.core.enums import OrderSide, OrderType
from trading_platform.core.events import EventBus
from trading_platform.core.logging import get_logger
from trading_platform.core.models import Order, QuoteTick


class TrailingStopState(StrEnum):
    """State machine for trailing stop orders."""
    PENDING = "pending"
    ACTIVE = "active"
    COMPLETED = "completed"
    CANCELED = "canceled"
    ERROR = "error"


TRAILING_STOP_TERMINAL_STATES = frozenset({
    TrailingStopState.COMPLETED,
    TrailingStopState.CANCELED,
    TrailingStopState.ERROR,
})


class TrailingStopChannel(StrEnum):
    """Event bus channels for trailing stop events."""
    TRAILING_STOP_PLACED = "trailing_stop.placed"
    TRAILING_STOP_UPDATED = "trailing_stop.updated"
    TRAILING_STOP_FILLED = "trailing_stop.filled"
    TRAILING_STOP_CANCELED = "trailing_stop.canceled"
    TRAILING_STOP_ERROR = "trailing_stop.error"
    TRAILING_STOP_STATE_CHANGE = "trailing_stop.state_change"


class TrailingStopOrder(BaseModel):
    """Represents a trailing stop order with ratchet behavior."""
    trailing_stop_id: str
    symbol: str
    quantity: Decimal
    trail_amount: Decimal | None = None
    trail_percent: Decimal | None = None
    current_stop_price: Decimal
    high_water_mark: Decimal
    stop_order_id: str | None = None
    state: TrailingStopState = TrailingStopState.PENDING
    fill_price: Decimal | None = None
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    completed_at: datetime | None = None

    model_config = {"ser_json_timedelta": "float"}


class TrailingStopManager:
    """Manages trailing stop orders with ratchet-up behavior.

    Monitors bid prices via EventBus quote events. When the price rises,
    the stop level is adjusted upward using cancel_and_replace. The stop
    level never moves down.
    """

    def __init__(self, event_bus: EventBus, exec_adapter: ExecAdapter | None = None) -> None:
        self._bus = event_bus
        self._exec = exec_adapter
        self._log = get_logger("orders.trailing_stop")
        self._trailing_stops: dict[str, TrailingStopOrder] = {}
        self._stop_to_trailing: dict[str, str] = {}
        self._monitored_symbols: set[str] = set()

    async def create_trailing_stop(
        self,
        symbol: str,
        quantity: Decimal,
        current_price: Decimal,
        trail_amount: Decimal | None = None,
        trail_percent: Decimal | None = None,
    ) -> TrailingStopOrder:
        """Create and place a trailing stop order.

        Args:
            symbol: Ticker symbol.
            quantity: Number of shares to sell when stop triggers.
            current_price: Current bid price (used to compute initial stop).
            trail_amount: Absolute dollar trail (mutually exclusive with trail_percent).
            trail_percent: Percentage trail as decimal, e.g. Decimal("0.05") for 5%.

        Returns:
            The created TrailingStopOrder.

        Raises:
            ValueError: If parameters are invalid.
            RuntimeError: If no execution adapter is configured.
        """
        if not self._exec:
            raise RuntimeError("No execution adapter configured")
        if trail_amount is None and trail_percent is None:
            raise ValueError("Either trail_amount or trail_percent is required")
        if trail_amount is not None and trail_percent is not None:
            raise ValueError("Cannot specify both trail_amount and trail_percent")
        if quantity <= 0:
            raise ValueError("quantity must be positive")
        if trail_amount is not None and trail_amount <= 0:
            raise ValueError("trail_amount must be positive")
        if trail_percent is not None and (trail_percent <= 0 or trail_percent >= 1):
            raise ValueError("trail_percent must be between 0 and 1 exclusive")

        # Calculate initial stop price
        stop_price = self._calculate_stop_price(current_price, trail_amount, trail_percent)

        trailing_stop_id = str(uuid.uuid4())
        ts = TrailingStopOrder(
            trailing_stop_id=trailing_stop_id,
            symbol=symbol,
            quantity=quantity,
            trail_amount=trail_amount,
            trail_percent=trail_percent,
            current_stop_price=stop_price,
            high_water_mark=current_price,
        )
        self._trailing_stops[trailing_stop_id] = ts

        await self._place_stop_order(ts)
        return ts

    def get_trailing_stop(self, trailing_stop_id: str) -> TrailingStopOrder | None:
        return self._trailing_stops.get(trailing_stop_id)

    def get_active_trailing_stops(self) -> list[TrailingStopOrder]:
        return [ts for ts in self._trailing_stops.values() if ts.state not in TRAILING_STOP_TERMINAL_STATES]

    async def cancel_trailing_stop(self, trailing_stop_id: str) -> bool:
        ts = self._trailing_stops.get(trailing_stop_id)
        if not ts or ts.state in TRAILING_STOP_TERMINAL_STATES:
            return False

        if ts.stop_order_id:
            try:
                await self._exec.cancel_order(ts.stop_order_id)
            except Exception as exc:
                self._log.warning("failed to cancel stop order", trailing_stop_id=trailing_stop_id, error=str(exc))

        await self._transition(ts, TrailingStopState.CANCELED)
        return True

    # ── Event Wiring ───────────────────────────────────────────────────

    async def wire_events(self) -> None:
        await self._bus.subscribe("quote", self._on_quote)
        await self._bus.subscribe("execution.order.filled", self._on_order_filled)
        await self._bus.subscribe("execution.order.cancelled", self._on_order_cancelled)

    async def unwire_events(self) -> None:
        await self._bus.unsubscribe("quote", self._on_quote)
        await self._bus.unsubscribe("execution.order.filled", self._on_order_filled)
        await self._bus.unsubscribe("execution.order.cancelled", self._on_order_cancelled)

    # ── Event Handlers ─────────────────────────────────────────────────

    async def _on_quote(self, channel: str, event: Any) -> None:
        """Monitor bid prices and ratchet stop levels upward."""
        if isinstance(event, QuoteTick):
            symbol = event.symbol
            bid_price = Decimal(str(event.bid_price))
        elif isinstance(event, dict) and "symbol" in event:
            symbol = event["symbol"]
            bid_price = Decimal(str(event.get("bid_price", 0)))
        else:
            return

        if symbol not in self._monitored_symbols:
            return

        for ts in self._trailing_stops.values():
            if ts.symbol != symbol or ts.state != TrailingStopState.ACTIVE:
                continue

            if bid_price <= ts.high_water_mark:
                continue

            # Price has risen — update high water mark and ratchet stop
            ts.high_water_mark = bid_price
            new_stop = self._calculate_stop_price(bid_price, ts.trail_amount, ts.trail_percent)

            if new_stop > ts.current_stop_price:
                old_stop = ts.current_stop_price
                ts.current_stop_price = new_stop
                await self._replace_stop_order(ts)
                self._log.info(
                    "trailing stop ratcheted up",
                    trailing_stop_id=ts.trailing_stop_id,
                    old_stop=str(old_stop),
                    new_stop=str(new_stop),
                    bid=str(bid_price),
                )

    async def _on_order_filled(self, channel: str, event: Any) -> None:
        order_id = event.get("order_id") if isinstance(event, dict) else None
        if not order_id:
            return

        if order_id in self._stop_to_trailing:
            trailing_stop_id = self._stop_to_trailing[order_id]
            ts = self._trailing_stops.get(trailing_stop_id)
            if ts and ts.state == TrailingStopState.ACTIVE:
                fill_price = event.get("fill_price") or event.get("avg_price")
                if fill_price is not None:
                    ts.fill_price = Decimal(str(fill_price))
                await self._transition(ts, TrailingStopState.COMPLETED)
                self._monitored_symbols.discard(ts.symbol)
                await self._bus.publish(TrailingStopChannel.TRAILING_STOP_FILLED, {
                    "trailing_stop_id": trailing_stop_id,
                    "symbol": ts.symbol,
                    "fill_price": str(ts.fill_price),
                    "stop_price": str(ts.current_stop_price),
                })

    async def _on_order_cancelled(self, channel: str, event: Any) -> None:
        """Handle cancelled stop orders — only act if we're in CANCELED state."""
        order_id = event.get("order_id") if isinstance(event, dict) else None
        if not order_id or order_id not in self._stop_to_trailing:
            return
        # Cancellation is expected during replace or user cancel — no action needed

    # ── Internal ───────────────────────────────────────────────────────

    @staticmethod
    def _calculate_stop_price(
        price: Decimal,
        trail_amount: Decimal | None,
        trail_percent: Decimal | None,
    ) -> Decimal:
        if trail_amount is not None:
            return price - trail_amount
        if trail_percent is not None:
            return price * (1 - trail_percent)
        raise ValueError("Either trail_amount or trail_percent required")

    async def _place_stop_order(self, ts: TrailingStopOrder) -> None:
        stop_order = Order(
            order_id=str(uuid.uuid4()),
            symbol=ts.symbol,
            side=OrderSide.SELL,
            order_type=OrderType.STOP,
            quantity=ts.quantity,
            stop_price=float(ts.current_stop_price),
        )
        ts.stop_order_id = stop_order.order_id
        self._stop_to_trailing[stop_order.order_id] = ts.trailing_stop_id

        try:
            await self._exec.submit_order(stop_order)
            await self._transition(ts, TrailingStopState.ACTIVE)
            self._monitored_symbols.add(ts.symbol)
            await self._bus.publish(TrailingStopChannel.TRAILING_STOP_PLACED, {
                "trailing_stop_id": ts.trailing_stop_id,
                "symbol": ts.symbol,
                "stop_price": str(ts.current_stop_price),
                "stop_order_id": stop_order.order_id,
            })
        except Exception as exc:
            self._log.error("stop order placement failed", trailing_stop_id=ts.trailing_stop_id, error=str(exc))
            await self._transition(ts, TrailingStopState.ERROR)
            await self._bus.publish(TrailingStopChannel.TRAILING_STOP_ERROR, {
                "trailing_stop_id": ts.trailing_stop_id,
                "error": f"stop placement failed: {exc}",
            })

    async def _replace_stop_order(self, ts: TrailingStopOrder) -> None:
        """Replace the resting stop order with updated stop price via cancel_and_replace."""
        old_order_id = ts.stop_order_id
        new_order_id = str(uuid.uuid4())

        # Use cancel_and_replace if the adapter supports it
        if hasattr(self._exec, "cancel_and_replace"):
            try:
                from public_api_sdk.models import CancelAndReplaceRequest
                request = CancelAndReplaceRequest(
                    order_id=old_order_id,
                    stop_price=ts.current_stop_price,
                )
                await self._exec.cancel_and_replace(request)
                # Update tracking: remove old, add new
                self._stop_to_trailing.pop(old_order_id, None)
                ts.stop_order_id = new_order_id
                self._stop_to_trailing[new_order_id] = ts.trailing_stop_id
                await self._bus.publish(TrailingStopChannel.TRAILING_STOP_UPDATED, {
                    "trailing_stop_id": ts.trailing_stop_id,
                    "symbol": ts.symbol,
                    "new_stop_price": str(ts.current_stop_price),
                    "new_order_id": new_order_id,
                })
                return
            except Exception:
                # Fall through to manual cancel + replace
                pass

        # Fallback: cancel and re-place
        try:
            await self._exec.cancel_order(old_order_id)
            self._stop_to_trailing.pop(old_order_id, None)

            stop_order = Order(
                order_id=new_order_id,
                symbol=ts.symbol,
                side=OrderSide.SELL,
                order_type=OrderType.STOP,
                quantity=ts.quantity,
                stop_price=float(ts.current_stop_price),
            )
            ts.stop_order_id = new_order_id
            self._stop_to_trailing[new_order_id] = ts.trailing_stop_id
            await self._exec.submit_order(stop_order)
            await self._bus.publish(TrailingStopChannel.TRAILING_STOP_UPDATED, {
                "trailing_stop_id": ts.trailing_stop_id,
                "symbol": ts.symbol,
                "new_stop_price": str(ts.current_stop_price),
                "new_order_id": new_order_id,
            })
        except Exception as exc:
            self._log.error("stop replacement failed", trailing_stop_id=ts.trailing_stop_id, error=str(exc))
            await self._transition(ts, TrailingStopState.ERROR)
            await self._bus.publish(TrailingStopChannel.TRAILING_STOP_ERROR, {
                "trailing_stop_id": ts.trailing_stop_id,
                "error": f"stop replacement failed: {exc}",
            })

    async def _transition(self, ts: TrailingStopOrder, new_state: TrailingStopState) -> None:
        old_state = ts.state
        ts.state = new_state
        if new_state in TRAILING_STOP_TERMINAL_STATES:
            ts.completed_at = datetime.now(timezone.utc)
        self._log.info(
            "trailing stop state change",
            trailing_stop_id=ts.trailing_stop_id,
            from_state=str(old_state),
            to_state=str(new_state),
        )
        await self._bus.publish(TrailingStopChannel.TRAILING_STOP_STATE_CHANGE, {
            "trailing_stop_id": ts.trailing_stop_id,
            "symbol": ts.symbol,
            "from_state": str(old_state),
            "to_state": str(new_state),
        })
